                                ...
                                {/if}
                                    <fieldset>
                                        ...
                                        {if !$product->isBundle() || !$bundleHasVariants}
                                            <button class="addtobasket btn btn-red" type="submit">
                                                <img src="{baseDir}/public/images/1px.gif" alt="" class="px1" />
                                                <span>{if $loyalty_exchange}{translate key="Exchange"}{else}dodaj do koszyka{/if}<span class="decor"></span></span>
                                            </button>
                                        {else} 
                                            <a href="{route function=$productRoute key=$product->product->product_id productName=$product->translation->name productId=$product->product->product_id}" class="btn btn-red">dodaj do koszyka</a>
                                        {/if}
                                    </fieldset>
                                </form>
                            {elseif $enable_availability_notifier && $product->isEnabledNotifier()}
